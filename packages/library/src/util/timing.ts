...
export const ensureHighResTime = (
  t: any, // This is built to replace a missing or old-style timestamp created
) =>
...
export class FrameTimeout {
  ...
  tick(frameTime = performance.now(), frameSynced = false) {
    ...
    if (remainingFrames <= thresholds[this.mode]) {
      ...
      this._animationFrameHandle = window.requestAnimationFrame((t) =>
        this.tick(t, true),
      )
