  ...
  return (
    <ButtonDropdown isOpen={dropdownOpen} toggle={() => setDropdownOpen(!dropdownOpen)}>
      <Button
        ...
        outline
        color="secondary"
        onClick={(e) => stateToDownload(context.store.getState(), { removeInternals: e.shiftKey })}
      >
        <Icon icon="save" weight="l" fallbackWeight="r" />
      </Button>
      <DropdownToggle caret split outline color="secondary" />
      <DropdownMenu>
          ...
          onUpload={([[content]]) => {
            try {
              ...
              if (Array.isArray(state)) {
                //TODO : do something once issue (https://github.com/FelixHenninger/lab.js/issues/51) get done
                alert('Array are not supporting yet')
              } else {
        ...
        <DropdownItem
          onClick={(e) =>
            stateToDownload(context.store.getState(), { removeInternals: e.shiftKey })
          }
        >
          Save
        </DropdownItem>
        ...
        <DropdownItem onClick={() => downloadStaticLocal(context.store.getState())}>
        ...
        <DropdownItem onClick={() => downloadStaticJatos(context.store.getState())}>
          JATOS <span className="text-muted">(Just Another Tool for Online Studies)</span>
        </DropdownItem>
        <DropdownItem onClick={() => downloadStaticPavlovia(context.store.getState())}>
          Pavlovia
        </DropdownItem>
        <DropdownItem
          onClick={() =>
        ...
        <DropdownItem onClick={() => downloadPsychDS(context.store.getState())}>
          Psych-DS sidecar template <span className="text-muted">(JSON-LD)</span>
        </DropdownItem>
      </DropdownMenu>
    </ButtonDropdown>
  )
